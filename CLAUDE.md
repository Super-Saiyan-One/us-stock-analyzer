...
│   │   ├── us-index-strategy/  # SPY/QQQ 抄底定投区 + 过热减仓区
...
│   ├── us-index-strategy-engine.ts # 美股大盘抄底/过热区间评分器
...
- 表 `us_index_strategy_config`: (id, name, params, updated_at) — SPY/QQQ 区间策略参数配置，`params` JSON 包含权重、阈值、指标周期、forward PE 实时闸门
- 关键函数:
