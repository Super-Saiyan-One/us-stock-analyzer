...
- Implemented and verified US Index Zones: `AutoQuantStock` research script/report now runs for SPY/QQQ/all, `us-stock-analyzer` has `/us-index-strategy` page/API/data layer/engine/config/i18n/navigation, all requested npm checks pass, local page/API smoke checks pass, and a Playwright browser smoke test confirmed SPY/QQQ switching, config apply/reset, and chart rendering.
- Reworked the US Index Zones default-parameter flow: AutoQuantStock now runs a SPY/QQQ joint parameter search with 2018/2020/2022/2024-2026 phase coverage and strict JSON output; Web defaults now use the research-selected `USIndexZoneResearchOpt` parameters and upgrade exact legacy 60/60 saved defaults at read time.
- Added Vercel resilience for BTC and US Index strategy pages: API routes now fall back to bundled static snapshots when live external data or SQLite config access fails, config saves degrade to volatile responses if SQLite is unavailable, and fallback snapshots are covered by `npm run test:fallbacks`.
- Re-optimized US Index Zones with split `panic_bottom` and `pullback_bottom` logic: AutoQuantStock now penalizes missing 2025H2 and 2026-04 pullback DCA windows, Web defaults are `fear=0.30 / valuation=0.15 / technical=0.40 / repair=0.15 / bottom=50 / panic=50 / pullback=34 / heat=52 / gap=5`, and refreshed fallback snapshots include pullback DCA reasons.
